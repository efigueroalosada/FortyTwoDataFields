...
class FortyTwoDataFieldsView extends Ui.DataField {
    ...
    function compute(info) {
        ...
        if (info has :currentSpeed) {
        	if (info.currentSpeed != null && info.currentSpeed > 0) {
        		speed = toMinsXKm(info.currentSpeed);
        	} else {
        		speed = "0:00";
        	}
        }
    ...
    function onUpdate(dc) {
        ...
        speedIcon.draw(dc);
        dc.drawText(x5, y0, Gfx.FONT_NUMBER_HOT, speed, Gfx.TEXT_JUSTIFY_RIGHT);
        
        timeIcon.draw(dc);
        dc.drawText(x3, y2, Gfx.FONT_NUMBER_HOT, toHMS(lapTime), Gfx.TEXT_JUSTIFY_RIGHT);
        
        distanceIcon.draw(dc);
        dc.drawText(x5, y2, Gfx.FONT_NUMBER_HOT, lapDistance.format("%.2f"), Gfx.TEXT_JUSTIFY_RIGHT);
	...
	function toMinsXKm(speed) {
		var pace = 60.0 / (speed*3.6);
		var decimalPart = pace - Math.floor(pace);
		var sec = decimalPart * 60;
    	return Math.floor(pace).format("%d")+":"+sec.format("%02d");
	}
